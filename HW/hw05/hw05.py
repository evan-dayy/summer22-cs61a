...
def depthTracker(t,v,d):
    if t.is_leaf():
      ...
      for branch in t.branches:
        depthTracker(branch,v,d+1) # avoid the new branches
      for _ in range(d):
        t.branches.append(Tree(v))
...
def duplicate_link(lnk, val):
    ...
    """
    "*** YOUR CODE HERE ***"
    if lnk is Link.empty or (lnk.first != val and lnk.rest is Link.empty):
      return lnk
    else:
      tar = lnk
    
    if tar.first == val:
      temp = Link(val, tar.rest)
      tar.rest = temp
      tar = tar.rest.rest
      duplicate_link(tar, val)
    else:
      tar = tar.rest
      duplicate_link(tar, val)
