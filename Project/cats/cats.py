...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    new = [x for x in paragraphs if select(x)]
    if k > len(new) - 1:
        return ''
    return new[k]
    # END PROBLEM 1
...
def accuracy(typed, reference):
    ...
    for i in range(len(typed_words)):
        for j in range(len(reference_words)):
            if i == j and typed_words[i] == reference_words[j]:
                rightcount +=1
    if not typed_words and not reference_words:
        return 100.0
    elif not typed_words and reference_words:
        return 0.0
    return rightcount / len(typed_words) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    helper = lambda w: diff_function(typed_word, w, limit)
    firstnewone = min(word_list, key=helper)
    if diff_function(typed_word, firstnewone, limit) > limit:
        return typed_word
    return firstnewone
    # END PROBLEM 5
...
def feline_fixes(typed, reference, limit):
    ...
    """
    # BEGIN PROBLEM 6 
    # typed, reference = typed[:min(len(typed),len(reference))], reference[:min(len(typed),len(reference))]

    if len(typed) == 1 or len(reference) == 1 or limit < 0:
        if typed[0] == reference[0]:
            return 0 + abs(len(reference) - len(typed))
        else:
            return 1 + abs(len(reference) - len(typed))
    else:
        if typed[0] == reference[0]:
            return 0 + feline_fixes(typed[1:], reference[1:], limit)
        else:
            limit -= 1
            return 1 + feline_fixes(typed[1:], reference[1:], limit)
    # END PROBLEM 6
...
def hidden_kittens(typed, reference, limit):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    def passthrough(typed, reference):
        if len(typed) < len(reference) or not helperfunc(typed, reference): # fail case
            return 0
        elif len(reference) == 1:
            return lettercount(typed, reference)
        else:
            index = typed.find(reference[0])
            return passthrough(typed[index+1:], reference) + passthrough(typed[index+1:], reference[1:])
    x = passthrough(typed, reference)
    if x == 0 or x > limit:
        return limit + 1
    return x
    # END PROBLEM 7

# Helper function of PROMLEM 7
################################################
def helperfunc(typed, reference):
        for i in range(len(reference)):
            req = reference[i]
            if typed.find(req) == -1:
                return False # if not find, then not calculate 
            typed = typed[typed.find(req):]
        return True
def lettercount(str, l):
    count = 0
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        newlst.append([times_per_player[i][j+1] - times_per_player[i][j] for j in range(len(times_per_player[i]) - 1)])
    d["times"] = newlst
    return d
    # END PROBLEM 9
