...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        proc = do_lambda_form(Pair(sym, body), env)
        env.define(signature.first, proc)
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        fst = scheme_eval(expressions.first, env)
        if is_scheme_false(fst):
            return False
        else:
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        l = bindings.first
        validate_form(l, 2, 2)
        eval = scheme_eval(l.rest.first, env)
        names = Pair(l.first,names)
        vals = Pair(eval, vals)
        bindings = bindings.rest
    
    # END PROBLEM 14
    if not validate_formals(names):
