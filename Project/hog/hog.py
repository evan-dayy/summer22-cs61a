...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            score_curr_turn_0 = take_turn(strategy0(score0, score1), score0, score1, dice, goal)
            score0 += score_curr_turn_0
            score0 += pigs_on_prime(score0, score1)
        else:
            score_curr_turn_1 = take_turn(strategy1(score1, score0), score1, score0, dice, goal)
            score1 += score_curr_turn_1
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    elif score1 > score0 and last_leader != 1:
...
def pigs_on_prime_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    if oink_points_strategy(score, opponent_score, threshold, num_rolls) == 0:
        return 0
    okpoint = oink_points(score, opponent_score)
    primepoint = pigs_on_prime(score+okpoint, opponent_score)
    if primepoint > 0:
        return 0
    if okpoint + primepoint >= threshold:
        return 0
    return num_rolls
    # END PROBLEM 11
